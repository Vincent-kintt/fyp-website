...
export async function acquireUserAILock(
  ...
  try {
    ...
    // MongoDB driver v6 returns the document directly (or null if no match).
    // Driver v4/v5 wrapped it as `{ value }`. Normalize across versions so
    // a future driver bump can't silently break the lock flow — same pattern
    // as lib/notifications/processDueReminders.js.
    return claimed?.value !== undefined ? claimed.value : claimed;
  } catch (err) {
