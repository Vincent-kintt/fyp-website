...
export async function processDueReminders({
  ...
  for (const reminder of dueReminders) {
    ...
    // Drivers v4/v5 wrapped results as { value: doc | null }. Normalize so a
    // future driver upgrade can't silently break the claim flow: an explicit
    // null/undefined check on the unwrapped doc treats both shapes uniformly.
    const claimedDoc =
      claimed?.value !== undefined ? claimed.value : claimed;
    if (claimedDoc === null || claimedDoc === undefined) continue;
    counters.processed++;
