...
// For notes list / NoteTopBar — relative phrase without suffix.
// Callers wrap with t("editedAgo", { time }) to attach localised "ago" copy.
export function formatRelativeTime(dateTime, locale) {
  if (!dateTime) return "";
  const d = new Date(dateTime);
  if (isNaN(d.getTime())) return "";
  const dfLocale = getDateFnsLocale(locale);
  return formatDistanceToNow(d, { addSuffix: false, locale: dfLocale });
}
