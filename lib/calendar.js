...
import { minutesToMs, isReminderCompleted } from "@/lib/utils";
...
export function groupOverlappingReminders(reminders) {
  ...
  for (const reminder of sorted) {
    ...
    const endA = startA + minutesToMs(durationA);
    ...
    for (const group of groups) {
      ...
      const overlapsGroup = group.some((other) => {
        ...
        const endB = startB + minutesToMs(durationB);
