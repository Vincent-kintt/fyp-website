  ...
  Object.assign(tools, {
    ...
    listReminders: tool({
      ...
      execute: async (params) => {
        ...
        if (tag) {
          if (query.$or) {
            query.$and = [{ $or: query.$or }, { tags: tag }];
            delete query.$or;
          } else {
            query.tags = tag;
          }
        ...
        if (subtasks !== undefined) {
          updateData.subtasks = Array.isArray(subtasks)
            ? subtasks.map((st, idx) => ({
                ...
                title: typeof st === "string" ? st : st.title,
                completed:
                  typeof st === "string" ? false : st.completed || false,
              }))
            : [];
        }
    ...
    analyzePatterns: tool({
      ...
      inputSchema: z.object({
        analysisType: z
          .enum(["frequency", "categories", "timing", "completion"])
    ...
    summarizeUpcoming: tool({
      ...
      inputSchema: z.object({
        ...
        groupBy: z
          ...
          .optional()
          .describe("Grouping method (optional)"),
      }),
      execute: async (params) => {
      ...
      toModelOutput: ({ output }) => {
        ...
        for (const [key, items] of Object.entries(output.summary)) {
          compactSummary[key] = items.map((r) => ({
            id: r._id?.toString(),
            title: r.title,
            dateTime: r.dateTime,
            status: r.status,
    ...
    exportReminders: tool({
      ...
      inputSchema: z.object({
        format: z.enum(["ics", "json", "csv"]).describe("Export format"),
        filter: z
          .enum(["today", "week", "month", "all"])
          .optional()
          .describe("Time filter"),
