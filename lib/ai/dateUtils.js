...
// Return a Date whose wall-clock parts (year/month/day/hour/minute/second), read via
// the standard `Date.prototype.get*` methods, match what a clock in `timezone` shows
// right now. The returned Date is NOT a faithful UTC instant — it's a server-local
// encoding of the user's wall clock, intended for libraries like chrono-node that
// reason about "today/tomorrow" using local Date methods on a reference date.
// Falls back to the server's own clock if timezone is falsy.
export function nowAsWallClockIn(timezone) {
  const now = new Date();
