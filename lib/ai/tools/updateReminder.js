...
export function createUpdateReminderTool(ctx) {
  ...
  return tool({
    ...
    execute: async (params) => {
      ...
      // status, so a concurrent web/AI write against the same baseline can't
      // also succeed. No match with isStatusChange=true means another writer
      // moved status while this tool was validating; surface a retryable
      // failure to the model. See CLAUDE.md anti-pattern entry.
      const filter = { _id: oid, userId };
      const isStatusChange = status !== undefined;
      if (isStatusChange) {
        filter.status = currentReminder.status || "pending";
      }
      ...
      if (!updated) {
        if (isStatusChange) {
          return {
            success: false,
            error: "Reminder status changed by another request, please retry",
          };
        }
        return { success: false, error: "Reminder not found" };
      }
