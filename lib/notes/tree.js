...
export function getProjection(flatItems, activeId, overId, dragOffsetX, indentWidth) {
  ...
  // Can only nest one level deeper than over-item if it has children; leaf items cap at their own depth
  const maxDepth = overItem.hasChildren ? overItem.depth + 1 : overItem.depth;
